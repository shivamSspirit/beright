...
/**
 * Check legacy delegate action (receiving delegated capital).
 *
 * Delegation products are retired from active scope; this path remains only for
 * historical credit consumers.
 */
function checkDelegateAction(
  ...
  if (credit.delegationCap === 0) {
    return {
      allowed: false,
      reason: 'Legacy delegation not eligible (need more track record)',
      currentTier: credit.poolAccessTier,
    };
  }
  ...
  if (amount && amount > credit.delegationCap) {
    return {
      allowed: false,
      reason: `Requested legacy delegation ($${amount}) exceeds cap ($${credit.delegationCap})`,
