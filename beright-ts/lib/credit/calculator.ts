...
/**
 * Calculate legacy delegation cap.
 *
 * Delegation/pool products are retired from active BeRight scope. This helper is
 * retained for historical credit consumers and should not drive new UX.
 *
 * Formula: baseCap × tierMultiplier × trackRecordMultiplier
 *
 * @param metrics - Forecaster performance metrics
 * @param config - Credit system configuration
 * @returns Legacy delegation cap in USDC
 */
export function calculateDelegationCap(
  metrics: CreditInputMetrics,
  config: CreditConfig = DEFAULT_CREDIT_CONFIG
): number {
  // No legacy delegation cap for new forecasters
  ...
  // No legacy delegation cap for restricted tier
