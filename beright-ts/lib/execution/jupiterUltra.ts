...
export class JupiterUltraClient {
  ...
  async getPrice(tokenMint: string): Promise<number> {
    try {
      const response = await fetch(
        `${this.v6ApiUrl}/price?ids=${tokenMint}&vsToken=EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v`
      );
  ...
  async getPrices(tokenMints: string[]): Promise<Map<string, number>> {
    ...
    try {
      ...
      const response = await fetch(
        `${this.v6ApiUrl}/price?ids=${ids}&vsToken=EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v`
      );
