  ...
  const computeResults = async (): Promise<MLMatchResult[]> => {
    ...
    const results: MLMatchResult[] = fabricResult.markets.map(market => ({
      ...
      arbitrage: market.arbitrageSpread && market.arbitrageSpread > 0.02 && market.arbitragePlatforms ? {
        buyPlatform: market.arbitragePlatforms.buyPlatform,
        buyPrice: market.platforms.find(p => p.platform === market.arbitragePlatforms?.buyPlatform)?.yesPrice || 0,
        sellPlatform: market.arbitragePlatforms.sellPlatform,
        sellPrice: market.platforms.find(p => p.platform === market.arbitragePlatforms?.sellPlatform)?.yesPrice || 0,
