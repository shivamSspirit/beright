...
import { useLeaderboard, useOnChainLeaderboard } from '@/hooks/useMarkets';
import { useRealLeaderboard } from '@/hooks/useRealLeaderboard';
import { useUser } from '@/hooks/useUnifiedUser';
import { Layers, Shield } from 'lucide-react';
import { PageWrapper } from '@/components/ui';
import styles from './leaderboard.module.css';
import { computeLeague, computeLevel, computeLeagueFromAccuracy } from '@/lib/leagues';
...
type MetricTab = 'SCORE' | 'PROFIT' | 'ACCURACY';
type TimeFilter = 'TODAY' | 'THIS WK' | 'THIS MO' | 'ALL TIME';
type DesktopMetric = 'Reputation' | 'Profit' | 'Accuracy';
...
export default function LeaderboardPage() {
  ...
  const { forecasters: onChainForecasters } = useOnChainLeaderboard();
  const { data: realLeaderboard } = useRealLeaderboard();

  // Mobile state
  const [activeTab, setActiveTab] = useState<MetricTab>('SCORE');
  const [activePill, setActivePill] = useState<TimeFilter>('THIS MO');

  // Desktop state
  const [activeMetric, setActiveMetric] = useState<DesktopMetric>('Reputation');
  ...
  const leaderboardData: LeaderboardEntry[] = useMemo(() => {
    ...
    const realEntries: LeaderboardEntry[] = realLeaderboard.map((entry, index) => ({
      ...
      penaltyFlags: entry.penaltyFlags,
      importedResolvedCount: entry.importedResolvedCount,
      nativeResolvedCount: entry.nativeResolvedCount,
    ...
    const onChainEntries: LeaderboardEntry[] = onChainForecasters.map((forecaster, index) => ({
      ...
      platform: 'beright',
      penaltyFlags: [],
      importedResolvedCount: 0,
      nativeResolvedCount: forecaster.totalPredictions || 0,
      trend: 'neutral' as const,
      change: null,
    ...
    const apiEntries: LeaderboardEntry[] = ((data?.leaderboard || []) as ApiLeaderboardEntry[]).map((entry, index) => {
      ...
      return {
        ...
        confidence: entry.confidence,
        platform: entry.platform || 'beright',
        penaltyFlags: entry.penaltyFlags || [],
        importedResolvedCount: entry.importedResolvedCount || 0,
        nativeResolvedCount: entry.nativeResolvedCount || 0,
        trend: (entry.change ?? 0) > 0 ? 'up' : (entry.change ?? 0) < 0 ? 'down' : 'neutral',
  ...
  const userStats = user as UserStats | null | undefined;
  ...
  const mobileTabs: MetricTab[] = ['SCORE', 'PROFIT', 'ACCURACY'];
  const mobilePills: TimeFilter[] = ['TODAY', 'THIS WK', 'THIS MO', 'ALL TIME'];
  const desktopMetrics: DesktopMetric[] = ['Reputation', 'Profit', 'Accuracy'];
  ...
  const getForecasterHref = (entry: LeaderboardEntry) =>
    `/forecasters/${encodeURIComponent(entry.walletAddress || entry.username)}`;
  const getScoreDisplay = (entry: LeaderboardEntry) =>
    entry.vaultScore !== undefined ? entry.vaultScore.toLocaleString() : '-';
  const getSourceLabel = (entry: LeaderboardEntry) =>
    entry.platform ? entry.platform.toUpperCase() : entry.isOnChainVerified ? 'BERIGHT' : 'IMPORTED';
    ...
    <PageWrapper showHeader={false} showFooter={false}>
      ...
      <main className={styles.mainGrid}>
        ...
        <aside className={styles.leftPanel}>
          ...
          {top3[0] && (
            <Link href={getForecasterHref(top3[0])} className={styles.rank1Card} aria-label={`Open ${getDisplayName(top3[0])} profile`}>
              ...
              <div className={styles.rank1Info}>
                ...
                <div className={styles.rank1Stats}>
                  <div className={styles.statBlock}>
                    <span className={styles.statLabel}>BeRight Score</span>
                    <span className={`${styles.statValue} ${styles.statValueGreen}`}>{getScoreDisplay(top3[0])}</span>
                  </div>
                  <div className={styles.statBlock}>
                    <span className={styles.statLabel}>Profit</span>
                    <span className={styles.statValue}>{top3[0].profit}</span>
                  </div>
                </div>
              </div>
            </Link>
          ...
          <div className={styles.runnersGrid}>
            {top3.slice(1).map((entry, idx) => (
              <Link
                key={entry.rank}
                href={getForecasterHref(entry)}
                className={`${styles.runnerCard} ${idx === 0 ? styles.runnerCardBorder : ''}`}
                aria-label={`Open ${getDisplayName(entry)} profile`}
              >
                ...
                <div>
                  ...
                  <div className={styles.runnerProfit}>S {getScoreDisplay(entry)} · {entry.profit}</div>
                </div>
        ...
        <div className={styles.mobilePodium}>
          {top3[1] && (
            <Link href={getForecasterHref(top3[1])} className={styles.podiumColumn} aria-label={`Open ${getDisplayName(top3[1])} profile`}>
              ...
              <div className={styles.podiumData}>
                ...
                <div className={styles.podiumValue}>S {getScoreDisplay(top3[1])}</div>
              </div>
            </Link>
          )}

          {top3[0] && (
            <Link href={getForecasterHref(top3[0])} className={styles.podiumColumn} aria-label={`Open ${getDisplayName(top3[0])} profile`}>
              ...
              <div className={styles.podiumData}>
                ...
                <div className={`${styles.podiumValue} ${styles.podiumValueGold}`}>S {getScoreDisplay(top3[0])}</div>
              </div>
            </Link>
          )}

          {top3[2] && (
            <Link href={getForecasterHref(top3[2])} className={styles.podiumColumn} aria-label={`Open ${getDisplayName(top3[2])} profile`}>
              ...
              <div className={styles.podiumData}>
                ...
                <div className={styles.podiumValue}>S {getScoreDisplay(top3[2])}</div>
              </div>
            </Link>
          )}
        </div>
        ...
        <section className={styles.rightPanel}>
          <div className={styles.listHeader}>
            <div>Rnk</div>
            <div>Trader</div>
            <div>BeRight Score</div>
            <div className={styles.hideTablet}>Accuracy</div>
            <div className={styles.hideTablet}>PnL</div>
            <div className={styles.listHeaderRight}>Chg</div>
          </div>

          {restOfList.map((entry) => (
            <Link
              key={`${entry.rank}-${entry.walletAddress || entry.username}`}
              href={getForecasterHref(entry)}
            ...
            >
              ...
              <div className={styles.rowProfit}>
                <span className={styles.rowScore}>S {getScoreDisplay(entry)}</span>
                <span className={styles.rowSource}>{getSourceLabel(entry)}</span>
              </div>
              <div className={`${styles.rowAccuracy} ${styles.hideTablet}`}>{entry.accuracy}%</div>
              <div className={`${styles.rowStreak} ${styles.hideTablet}`}>{entry.profit}</div>
