...
export function useUser(): UnifiedUserContextType {
  ...
  useEffect(() => {
    ...
    // Also check on bridge/storage events so auth transitions do not wait for polling.
    window.addEventListener('beright-wallet-state', checkWalletState);
    window.addEventListener('storage', checkWalletState);

    return () => {
      clearInterval(interval);
      window.removeEventListener('beright-wallet-state', checkWalletState);
